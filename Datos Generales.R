  ...
  for (archivo in list.files(paste("Data", folder, sep ="/" ))) {
    ...
    for (sheet in 1:length(excel_sheets(paste("Data", folder, archivo, sep = "/")))) {
      ...
      temp_table<- temp_table  %>% filter(complete.cases(temp_table)) %>% mutate(Llegada_Anterior= lag(Llegada,1), Final_Anterior= lag(Final,1)) %>% 
        ...
        # Transformar a minutos en el Tiempo de Cola se estiman 6 segundos para considerar las mala toma de datos
        mutate(T_Llegadas=T_Llegadas/dminutes(1), T_Cola= ifelse(T_Cola/dminutes(1) < 0.1, 0, T_Cola/dminutes(1)), T_Servicio=T_Servicio/dminutes(1), T_Sistema= T_Sistema/dminutes(1),
